...
/*TODO :
 * if wrong password, should not throw exception closing the whole program
 * change everything which is critical into exceptions
 * command kick
 * command invite
 * ban?
 * kill?
 * nickname -> check if already taken and add symbol
 * nickname + user -> check if valid
 * join channel -> topic sent (to be checked)
